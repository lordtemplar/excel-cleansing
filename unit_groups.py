...
# Define unit groups with group names
unit_groups = {
    "กลุ่มที่ 1 พล.1 รอ.": [
    ...
    "กลุ่มที่ 3 พล.ร.9": [
        "พล.ร.9", "ร.9 พัน.1", "ร.9 พัน.2", "ร.9 พัน.3", "ร.19 พัน.1", "ร.19 พัน.2", "ร.19 พัน.3",
    ...
    "กลุ่มที่ 7 บชร.1": [
        "พัน.สพ.กระสุน 21 บชร.1", "พัน.สบร.21 บชร.1", "หน่วยฝึกที่ 1", "หน่วยฝึกที่ 2", "หน่วยฝึกที่ 3", "พัน.สห.11"
    ],
    "กลุ่มที่ 8 มทบ.": [
        "มทบ.12", "มทบ.13", "มทบ.14", "มทบ.15", "มทบ.16", "มทบ.17", "มทบ.18", "มทบ.19"
    ],
    "กลุ่มที่ 9 บก.ทภ.1": [
...
def count_by_group(df, unit_groups):
    ...
    # Convert to DataFrame and add total row
    group_counts_df = pd.DataFrame(list(group_counts.items()), columns=["กลุ่ม", "จำนวนคน"])
    total = group_counts_df["จำนวนคน"].sum()
    group_counts_df = pd.concat([group_counts_df, pd.DataFrame([["รวม", total]], columns=["กลุ่ม", "จำนวนคน"])], ignore_index=True)
